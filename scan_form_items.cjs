...
function analyzeFormItemAt(fileContent, startPos) {
  ...
  while (pos < fileContent.length) {
    ...
    if (nextOpen !== -1 && nextOpen < nextClose) {
      ...
      if (innerTagEnd > 0 && fileContent[innerTagEnd - 1] !== '/') {
        depth++;
      }
...
function findDirectChildren(inner) {
  ...
  while (i < inner.length) {
    ...
    if (textMatch) {
      ...
      if (txt.length > 0) {
        children.push({ type: 'text', content: txt.substring(0, 100) });
      }
...
for (const file of files) {
  ...
  while (pos < content.length) {
    ...
    if (result) {
      ...
      // Multi-element violation (多个直接子元素)
      ...
      // Single child that is a layout container
...
console.log('\n========== 高优先级：Form.Item 带 name 但有多个直接子元素（一定触发 antd 警告） ==========\n');
...
for (const v of multiChildViolations) {
  ...
  console.log(`\n📁 src/pages/${relPath}:${v.line}`);
  console.log(`\u251c\u2500 标签: ' + v.formItemLine);
  console.log(`\u251c\u2500 直接子元素:');
  for (const c of v.children) {
    if (c.type === 'tag') console.log(`\u2502     \u251c\u2500 <' + c.name + '>');
    else if (c.type === 'expression') console.log(`\u2502     \u251c\u2500 {' + c.content.substring(0, 60) + '}');
    else if (c.type === 'text') console.log(`\u2502     \u251c\u2500 "' + c.content.substring(0, 60) + '"');
  }
  console.log(`\u2514\u2500 代码预览: ${v.innerPreview.substring(0, 200)}`);
}

console.log('\n\n========== 中优先级：Form.Item 带 name 单一子元素是布局容器（Row/Col/Space/div 等，Form 可能无法正常接管） ==========\n');
