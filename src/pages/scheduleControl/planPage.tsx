...
const PlanPanel: React.FC<PlanPageProps> = () => {
  ...
  const handleReviewSubmit = (payload: { status: '通过' | '驳回'; comment: string; reviewer: string }) => {
    ...
    if (payload.status === '驳回') {
      setList(prev => prev.map(item => item.key === key ? { ...item, status: '已驳回' as SCPlanStatus } : item))
      message.success('已驳回')
    } else {
      setList(prev => prev.map(item => item.key === key ? { ...item, status: '已审批' as SCPlanStatus } : item))
