...
const [approvalMap, setApprovalMap] = usePersistedState<Record<string, ApprovalRecord[]>>('acceptanceFiling-reportPage-approval', {})
  ...
  const handleDelete = (key: string) => {
    ...
    if (deletedItem) {
      addAuditLog(currentUser.name, '验收归档', '删除', deletedItem.title, '验收报告', `删除验收报告：${deletedItem.title}（编号：${deletedItem.code}）`)
    }
  }
  ...
  const handleAddOk = () => {
    addForm.validateFields().then(values => {
      ...
      addAuditLog(currentUser.name, '验收归档', '新增', newItem.title || newItem.code, '验收报告', `新增验收报告：${newItem.title}（编号：${newItem.code}）`)
    })
  }
  ...
  const handleEditOk = () => {
    editForm.validateFields().then(values => {
      if (currentItem) {
        ...
        addAuditLog(currentUser.name, '验收归档', '编辑', currentItem.title, '验收报告', `编辑验收报告：${currentItem.title}（编号：${currentItem.code}）`)
      }
    })
  }
  ...
  const handleReset = () => {
    ...
    setList(getPersistedData<ACReportItem[]>('accept-report') ?? list)
  }
  ...
  const handleReviewSubmit = (payload: { status: '通过' | '驳回'; comment: string; reviewer: string }) => {
    ...
    if (payload.status === '驳回') {
      setList(prev => { const r = prev.map(item => item.key === key ? { ...item, status: '待审批' as ACReportStatus } : item); return r })
      message.success('已驳回，返回待审批')
    } else {
      const newStatus: ACReportStatus = currentItem.status === '待审批' ? '一审通过' : '已审批'
      setList(prev => { const r = prev.map(item => item.key === key ? { ...item, status: newStatus } : item); return r })

      // 当竣工验收类型的报告最终审批通过时，同步将对应项目的状态更新为"已验收"
      if (newStatus === '已审批' && currentItem.type === '竣工验收') {
        ...
        message.success(`竣工验收终审已通过，项目状态已同步为"已验收"`)
        addAuditLog(currentUser.name, '验收归档', '审批', currentItem.title, '验收报告', `审批通过：${currentItem.title}（编号：${currentItem.code}），类型：${currentItem.type}，状态：已审批`)
      } else if (newStatus === '已审批') {
        message.success('终审已通过')
        addAuditLog(currentUser.name, '验收归档', '审批', currentItem.title, '验收报告', `审批通过：${currentItem.title}（编号：${currentItem.code}），状态：已审批`)
      } else {
        message.success('一审通过，等待总监理工程师终审')
        addAuditLog(currentUser.name, '验收归档', '审批', currentItem.title, '验收报告', `审批通过：${currentItem.title}（编号：${currentItem.code}），状态：一审通过`)
      }
    }
    setIsReviewModalVisible(false)
    setCurrentItem(null)
